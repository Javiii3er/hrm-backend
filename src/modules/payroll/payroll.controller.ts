...
export class PayrollController {
  async createPayroll(req: Request, res: Response) {
    try {
      ...
      sendSuccess(res, result, 201, { message: 'Nómina creada exitosamente' });
    } catch (error: unknown) {
      const errorMessage = getErrorMessage(error);
      switch (errorMessage) {
        case 'INVALID_DATE_RANGE':
          sendError(res, 'INVALID_DATE_RANGE', 'Rango de fechas inválido', 422);
          break;
        case 'OVERLAPPING_PERIOD':
          sendError(res, 'OVERLAPPING_PERIOD', 'Periodo se superpone con otra nómina', 409);
          break;
        case 'DEPARTMENT_NOT_FOUND':
          sendError(res, 'DEPARTMENT_NOT_FOUND', 'Departamento no encontrado', 404);
          break;
        default:
          sendError(res, 'CREATE_ERROR', 'Error al crear nómina', 500);
      }
    }
  }

  async createIndividualPayroll(req: Request, res: Response) {
    try {
      const { employeeId } = req.params;
      const result = await payrollService.createIndividualPayroll(employeeId, req.body);
      sendSuccess(res, result, 201, { message: 'Nómina individual creada exitosamente' });
    } catch (error: unknown) {
      const errorMessage = getErrorMessage(error);
      switch (errorMessage) {
        ...
        case 'OVERLAPPING_PERIOD':
          sendError(
            res,
            'OVERLAPPING_PERIOD',
            'El empleado ya tiene una nómina en este rango de fechas',
            409
          );
          break;
        default:
          sendError(res, 'CREATE_ERROR', 'Error al crear nómina individual', 500);
      }
    }
  }
  ...
  async getPayrolls(req: Request, res: Response) {
    try {
      const result = (await payrollService.getPayrolls(req.query as any)) as unknown as {
        data: any[];
        total: number;
      };
  ...
  async getPayroll(req: Request, res: Response) {
    try {
      ...
      sendSuccess(res, result, 200, { message: 'Detalles de nómina obtenidos' });
    } catch (error: unknown) {
  ...
  async generateItems(req: Request, res: Response) {
    try {
      ...
      sendSuccess(res, result, 200, { message: 'Items de nómina generados' });
    } catch (error: unknown) {
      const errorMessage = getErrorMessage(error);
      switch (true) {
        ...
        case errorMessage.startsWith('EMPLOYEE_NOT_FOUND'):
          ...
          break;
        default:
          sendError(res, 'GENERATION_ERROR', 'Error al generar items', 500);
      }
    }
  }
  ...
  async finalizePayroll(req: Request, res: Response) {
    try {
      ...
      sendSuccess(res, result, 200, { message: 'Nómina finalizada exitosamente' });
    } catch (error: unknown) {
      const errorMessage = getErrorMessage(error);
      switch (errorMessage) {
        ...
        case 'PAYROLL_ALREADY_FINALIZED':
          sendError(res, 'PAYROLL_ALREADY_FINALIZED', 'Nómina ya finalizada', 422);
          break;
        case 'NO_PAYROLL_ITEMS':
          sendError(res, 'NO_PAYROLL_ITEMS', 'No hay items en la nómina', 422);
          break;
        default:
          sendError(res, 'FINALIZE_ERROR', 'Error al finalizar nómina', 500);
      }
    }
  }
  ...
  async deletePayroll(req: Request, res: Response) {
    try {
      ...
      sendSuccess(res, result, 200, { message: 'Nómina eliminada exitosamente' });
    } catch (error: unknown) {
      const errorMessage = getErrorMessage(error);
      switch (errorMessage) {
        case 'PAYROLL_NOT_FOUND':
          sendError(res, 'PAYROLL_NOT_FOUND', 'Nómina no encontrada', 404);
          break;
        case 'PAYROLL_NOT_DELETABLE':
          sendError(res, 'PAYROLL_NOT_DELETABLE', 'Solo nóminas en borrador pueden eliminarse', 422);
          break;
        default:
          sendError(res, 'DELETE_ERROR', 'Error al eliminar nómina', 500);
      }
    }
  }
  ...
  async getStats(req: Request, res: Response) {
    try {
      ...
      sendSuccess(res, result, 200, { message: 'Estadísticas de nóminas obtenidas' });
    } catch (error: unknown) {
