...
export class ApiClient {
  ...
  private async request<T>(endpoint: string, options: RequestInit = {}): Promise<ApiResponse<T>> {
    // CORRECCIÓN: Usar concatenación de strings para evitar el error de sintaxis en la cadena de texto.
    ...
    if (!response.ok) {
      // Manejo de errores que coincida con la interfaz ApiError
...
`;
  ...
  writeFileSync('./src/types/api.gen.ts', typesContent);
